...
public class RevokesFromContractorTest
{
	...
	public void CustomersFromContractor_PropertiesSetCorrectly_ReturnsTrue()
	{
		...
		Assert.True(revokesFromContractor.ArchiveProcessedFolder ==
	...
	public void ReadFile_ValidFiles_ReturnsValidRevokeViewModel()
	{
		...
		if (!Directory.Exists(revokesFromContractor.ArchiveFolder))
		{
			Directory.CreateDirectory(revokesFromContractor.ArchiveFolder);
		}

		File.Copy("IncomingFiles\\RevokesFromContractorTests\\ReadFile1.txt",
			$"{revokesFromContractor.ArchiveFolder}ReadFile1.txt", true);
		File.Copy("IncomingFiles\\RevokesFromContractorTests\\ReadFile2.txt",
			$"{revokesFromContractor.ArchiveFolder}ReadFile2.txt", true);
	...
	public void ReadFile_InvalidFile_ThrowsException()
	{
		...
		File.Copy("IncomingFiles\\RevokesFromContractorTests\\ReadFile3.txt",
			$"{revokesFromContractor.ArchiveFolder}ReadFile3.txt", true);
