...
public class CustomersToPrintContractorTests
{
	...
	public void CustomerToPrintContractor_PropertiesSetCorrectly()
	{
		...
		//Act
		ICustomersToPrintContractor customersFromContractor = new CustomersToPrintContractor(GetMockMediator().Object);
	...
	public void WriteFile_ValidCustomer_ReturnsTrue()
	{
		//Arrange
		ICustomersToPrintContractor customersFromContractor = new CustomersToPrintContractor(GetMockMediator().Object);
		Directory.CreateDirectory(Path.GetDirectoryName(customersFromContractor.ArchiveFileFullPath));
		...
		//Act
		bool isSuccessful = customersFromContractor.WriteFile(customers);

		//Assert
		Assert.True(isSuccessful);
		Assert.True(File.Exists(customersFromContractor.ArchiveFileFullPath));

		string line1 = File.ReadLines(customersFromContractor.ArchiveFileFullPath).First();
