...
public class ProcessWorkflowCommandHandler(IMediator mediator, IRevokesFromContractor incomingFiles, IUnitOfWork unitOfWork) : IRequestHandler<ProcessWorkflowCommand>
{
	public async Task Handle(ProcessWorkflowCommand request, CancellationToken cancellationToken)
	{
		try
		{
			...
			{
				await mediator.Send(new CreateLogCommand($"{incomingFiles.BatchName} - No files to decrypt.", LogType.Warning), cancellationToken);
				return;
			}
			...
			await mediator.Send(new CreateLogCommand($"{incomingFiles.BatchName} - Successfully imported revokes data.", LogType.Information), cancellationToken);
		}
		catch (Exception e)
		{
