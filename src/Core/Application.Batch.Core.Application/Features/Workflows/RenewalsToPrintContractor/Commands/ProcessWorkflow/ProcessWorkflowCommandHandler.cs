...
public class ProcessWorkflowCommandHandler(
	IMediator mediator,
	IRenewalsToPrintContractor outgoingFiles,
	IUnitOfWork unitOfWork) : IRequestHandler<ProcessWorkflowCommand>
{
	public async Task Handle(ProcessWorkflowCommand request, CancellationToken cancellationToken)
	{
		try
		{
			...
			await mediator.Send(
				new CreateLogCommand(
					$"{outgoingFiles.BatchName} - Begin generating file for Renewals To Print Contractor.",
			...
			{
				await mediator.Send(
					new CreateLogCommand($"{outgoingFiles.BatchName} - No customers were found. Files not generated.",
			...
			if (!outgoingFiles.WriteFiles(customers))
			{
				await mediator.Send(
					new CreateLogCommand(
						$"{outgoingFiles.BatchName} - Failed to write files for Renewals To Print Contractor.",
			...
			if (!outgoingFiles.DoArchiveFilesExist())
			{
				await mediator.Send(
					new CreateLogCommand(
						$"{outgoingFiles.BatchName} - Failed to find files for Renewals To Print Contractor.",
						LogType.Error), cancellationToken);
				return;
			}
			...
			if (outgoingFiles.DoArchiveGpgFilesExist())
			{
				...
				await mediator.Send(
					new CreateLogCommand($"{outgoingFiles.BatchName} - Successfully completed workflow.",
						LogType.Information), cancellationToken);
			...
			else
			{
				await mediator.Send(
					new CreateLogCommand(
						$"{outgoingFiles.BatchName} - Failed to encrypt file for Renewals To Print Contractor.",
						LogType.Error), cancellationToken);
		...
		catch (Exception e)
		{
			await mediator.Send(
				new CreateLogCommand(
					$"{outgoingFiles.BatchName} - Error occurred generating file for Renewals To Print Contractor.  Error message: {e.Message}",
					LogType.Error), cancellationToken);
		}
		finally
		{
			int fileRetentionLengthInMonths =
				Convert.ToInt32(await mediator.Send(new GetConfigurationByKeyQuery("FileRetentionPeriodInMonths"),
					cancellationToken));
			await outgoingFiles.CleanUpArchiveFolder(fileRetentionLengthInMonths);
			await mediator.Send(
				new CreateLogCommand($"{outgoingFiles.BatchName} - End generating file for Customer List.",
					LogType.Information), cancellationToken);
		}
	}
}
